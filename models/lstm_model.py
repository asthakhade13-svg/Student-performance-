...
class StudentMultiTaskLSTM(nn.Module):
    def __init__(self, seq_features=7, hidden_dim=16, num_layers=1):
        super(StudentMultiTaskLSTM, self).__init__()
    ...
    def forward(self, x):
        ...
        shared_out = self.shared_fc(last_hidden)
...
def train_pytorch_model(df, model_path):
    ...
    if cv >= 2:
        ...
        for train_idx, val_idx in kf.split(seq_data):
            ...
            for epoch in range(250):
                optimizer.zero_grad()
                pred_reg, pred_clf = model(torch.tensor(s_tr, dtype=torch.float32))
            ...
            model.eval()
            with torch.no_grad():
                pred_reg_val, pred_clf_val = model(torch.tensor(s_val, dtype=torch.float32))
    ...
    final_model = StudentMultiTaskLSTM()
    ...
    for epoch in range(250):
        optimizer.zero_grad()
        pred_reg, pred_clf = final_model(torch.tensor(seq_data_scaled, dtype=torch.float32))
